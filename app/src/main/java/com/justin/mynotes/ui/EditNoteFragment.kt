...
import com.justin.mynotes.viewModels.EditNoteViewModel

class EditNoteFragment : Fragment() {
    private lateinit var binding: FragmentAddEditNoteBinding
    private val viewModel: EditNoteViewModel by viewModels {
        EditNoteViewModel.Provider((requireContext().applicationContext as MyApplication).noteRepository)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val navArgs: EditNoteFragmentArgs by navArgs()
        viewModel.getNoteById(navArgs.id)
        viewModel.note.observe(viewLifecycleOwner) {
            binding.apply {
                tvHeader.text = "Update Note"
                etTitle.setText(it.title)
                etDetails.setText(it.details)
                btnSubmit.text = "Save"

                when (it.color) {
                    "#FFEB3B" -> rbOne.isChecked = true
                    "#FDA286" -> rbTwo.isChecked = true
                    "#EF9DD4" -> rbThree.isChecked = true
                    "#F35E91" -> rbFour.isChecked = true
                    else -> rbFive.isChecked = true
                }
            }
        }
        binding.run {
            btnSubmit.setOnClickListener {
                ...
                if (validate(title, details)) {
                    ...
                    setFragmentResult("from_edit_note", bundle)
                    viewModel.editNote(navArgs.id, Note(navArgs.id, title, details, color))
                    Toast.makeText(requireContext(), "Note updated", Toast.LENGTH_SHORT).show()
                    NavHostFragment.findNavController(requireParentFragment()).popBackStack()
                    Log.d("Edited note", "${navArgs.id}")
                } else {
                    val snackbar =
